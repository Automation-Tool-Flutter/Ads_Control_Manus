...
export default function AskAdsPage() {
  ...
  return (
    <PageContainer className="pb-28">
      <ControlHeader
        breadcrumbs={[{ label: 'Accounts', href: '/accounts' }, { label: accountName, href: `/accounts/${accountId}` }, { label: 'Ask your Ads' }]}
      ...
      {chat.dataState.status === 'loading' && <div className="rounded-xl border border-border bg-bg-card p-10 text-center"><div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-border border-t-accent" /><p className="mt-3 text-sm font-semibold text-text-secondary">Loading campaign, ad set, and ad data for both periods…</p></div>}
      ...
      {snapshot && <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_18rem]">
        <section className="flex min-h-[65vh] min-w-0 flex-col overflow-hidden rounded-xl border border-border bg-bg-card">
          ...
          <div ref={scroll.container} onScroll={scroll.onScroll} role="log" aria-label="AI conversation" aria-live="polite" className="full-chat-log min-h-0 flex-1 space-y-5 overflow-y-auto p-4 sm:p-5">
            ...
            {chat.messages.map(message => message.role === 'user' ? (
              <div key={message.id} className="flex justify-end"><div className="max-w-[85%] rounded-2xl rounded-br-md bg-accent px-4 py-3 text-sm leading-relaxed text-white">{message.content}</div></div>
            ) : (
              <article key={message.id} className="max-w-4xl rounded-2xl rounded-bl-md border border-border bg-bg-primary/45 p-4">
                <div className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-accent text-xs font-black text-white">AI</span><p className="text-xs font-black uppercase text-accent">Ads analyst</p></div>
                <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-text-primary">{message.result.answer}</p>

                {message.result.tables.map((table, tableIndex) => <div key={`${table.title}-${tableIndex}`} className="mt-4 overflow-hidden rounded-lg border border-border bg-bg-card"><p className="border-b border-border bg-bg-secondary/60 px-3 py-2 text-xs font-bold text-text-primary">{table.title}</p><div className="overflow-x-auto"><table className="w-full min-w-[32rem] text-left text-xs"><thead><tr>{table.columns.map(column => <th key={column} className="border-b border-border px-3 py-2 font-black uppercase text-text-muted">{column}</th>)}</tr></thead><tbody>{table.rows.map((row, rowIndex) => <tr key={rowIndex} className="border-b border-border/60 last:border-0">{table.columns.map((_, cellIndex) => <td key={cellIndex} className="px-3 py-2.5 text-text-secondary">{row.cells[cellIndex] ?? '—'}</td>)}</tr>)}</tbody></table></div></div>)}
                ...
                {message.result.caveats.length > 0 && <details className="mt-4 text-xs text-text-muted"><summary className="cursor-pointer font-bold">Data limitations ({message.result.caveats.length})</summary><ul className="mt-2 space-y-1">{message.result.caveats.map((item, index) => <li key={index}>• {item}</li>)}</ul></details>}
              </article>
            ))}
            {chat.isSending && <div className="flex items-center gap-2 text-sm text-text-muted"><span className="h-2 w-2 animate-pulse rounded-full bg-accent" /><span>AI is comparing the data…</span></div>}
          ...
          <form onSubmit={submit} className="border-t border-border bg-bg-card p-3 sm:p-4"><div className="flex items-end gap-2"><textarea aria-label="Enter your AI question" value={question} onChange={e => setQuestion(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey && !e.nativeEvent.isComposing && window.matchMedia('(hover: hover) and (pointer: fine)').matches) { e.preventDefault(); e.currentTarget.form?.requestSubmit(); } }} rows={2} maxLength={1500} placeholder="For example: Compare sales campaigns with the previous period…" className="min-w-0 min-h-[52px] flex-1 resize-none rounded-xl border border-border bg-bg-primary px-3 py-2.5 text-sm text-text-primary outline-none focus:border-accent" /><button disabled={!question.trim() || chat.isSending} className="meta-action meta-action-primary h-[52px] px-5 disabled:opacity-40">Send</button></div><p className="mt-1.5 text-[10px] text-text-muted">Tap Send when ready · Enter adds a line on mobile · Review AI recommendations before applying.</p></form>
        </section>

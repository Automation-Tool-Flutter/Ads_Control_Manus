  ...
  return (
    <main className="px-3 py-4 sm:px-6 sm:py-10">
      <section className="mx-auto grid max-w-[1400px] gap-4 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="neo-panel relative min-h-[620px] overflow-hidden p-6 sm:p-10">
          <div className="absolute right-[-8rem] top-[-8rem] h-80 w-80 rounded-full bg-accent/40 blur-3xl" />
          <div className="absolute bottom-[-7rem] left-[20%] h-72 w-72 rounded-full bg-status-green/25 blur-3xl" />
          <div className="relative flex h-full flex-col justify-between">
            <div>
              <div className="inline-flex rounded-full border border-white/15 bg-white/10 px-4 py-2 text-xs font-black uppercase tracking-[0.22em] text-white/70">
                Meta operations OS
              </div>
              <h1 className="mt-8 max-w-4xl text-6xl font-black leading-[0.88] text-white sm:text-7xl lg:text-8xl">
            ...
            <div className="mt-10 grid gap-3 sm:grid-cols-[auto_auto_1fr] sm:items-center">
              <Link
                href="/login"
                className="mobile-action inline-flex items-center justify-center rounded-full bg-white px-6 text-sm font-black text-text-primary shadow-2xl"
              >
                Connect Facebook
              </Link>
              <a
                href="#workzones"
                className="mobile-action inline-flex items-center justify-center rounded-full border border-white/20 px-6 text-sm font-black text-white/82"
              >
                View work zones
              </a>
        ...
        <div className="grid gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {stats.map((stat) => (
              <div key={stat.label} className="soft-card rounded-[2rem] p-5">
      ...
      <section id="workzones" className="mx-auto mt-4 max-w-[1400px]">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {features.map((feature, index) => (
            <article
              key={feature.title}
              className={`rounded-[2rem] border border-border p-5 shadow-xl shadow-slate-900/5 ${
                index === 0
                  ? "bg-accent text-white"
                  : "bg-white/82 text-text-primary backdrop-blur"
              }`}
            >
  ...
  return (
    <section className="soft-card rounded-[2rem] p-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="h-12 w-12 rounded-2xl bg-bg-secondary" />
      </div>
